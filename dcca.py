...
import math
from DSPFP import DSPFP, greedy_assignment, lapjv_assignment
...
def CDPA(Y_1,Y_2, r_1=None, r_2=None, r_12=None, method=None, P_set = 1, P_mat=None, assignment=1):
    ...
    D_1_hat = X_1_hat - C_1_hat #slightly different with those from dCCA, not enforced by the Euclidean orthogonality
    D_2_hat = X_2_hat - C_2_hat #
    
    Q_1, _, _ = sp.linalg.svd(B_1, full_matrices=False)
    Q_2, _, _ = sp.linalg.svd(B_2, full_matrices=False)
    
    if p_1 > p_2:
        Q_2 = np.concatenate((Q_2, np.zeros([p_1-p_2,r_12])), axis=0)
        B_2 = np.concatenate((B_2, np.zeros([p_1-p_2,r_12])), axis=0)
    elif p_1 < p_2:
        Q_1 = np.concatenate((Q_1, np.zeros([p_2-p_1,r_12])), axis=0)
        B_1 = np.concatenate((B_1, np.zeros([p_2-p_1,r_12])), axis=0)
    
    
    if P_set == 0:
        U_B1, lambda_B, U_B2 = sp.linalg.svd(Q_1.T @ Q_2, full_matrices=True)
        U_B2 = U_B2.T
        V_B2 = Q_2 @ U_B2
    else:    
        if P_mat is None:
            ...
            M_min = np.min([M_1, M_2])
            
            M_1 -= M_min
            M_2 -= M_min
              
            M_1_nondiag = M_1 - np.diag(M_1.diagonal())
            M_2_nondiag = M_2 - np.diag(M_2.diagonal())
             
            P_mat = DSPFP(M_1_nondiag, M_2_nondiag, M_1.diagonal(), M_2.diagonal())
            
            if assignment == 1:
                P_mat = greedy_assignment(P_mat)
            else:
                P_mat = lapjv_assignment(P_mat)
        
        U_B1, lambda_B, U_B2 = sp.linalg.svd(Q_1.T @ P_mat @ Q_2, full_matrices=True)
        U_B2 = U_B2.T
        V_B2 = P_mat @ Q_2 @ U_B2
    ...
    C_B = (V_B1+V_B2) @ np.diag( 0.5 * (1-  ((1-lambda_B)/(1+lambda_B))**0.5 ) )
    
    if P_set == 0:
        C_mat = np.sqrt(n) * 0.5 * C_B @ (V_B1.T @ B_1/norm(X_1_hat,'fro') + V_B2.T @ B_2/norm(X_2_hat,'fro')) @ C_base   
        C_mat_neg = np.sqrt(n) * 0.5 * C_B @ (V_B1.T @ B_1/norm(X_1_hat,'fro') - V_B2.T @ B_2/norm(X_2_hat,'fro')) @ C_base 
        #C_mat_X2 = C_mat[:p_2,:]
    else:
        C_mat = np.sqrt(n) * 0.5 * C_B @ (V_B1.T @ B_1/norm(X_1_hat,'fro') + V_B2.T @ P_mat @ B_2/norm(X_2_hat,'fro')) @ C_base
        C_mat_neg = np.sqrt(n) * 0.5 * C_B @ (V_B1.T @ B_1/norm(X_1_hat,'fro') - V_B2.T @ P_mat @ B_2/norm(X_2_hat,'fro')) @ C_base
        #C_mat_X2 = P_mat.T[:p_2,:] @ C_mat
        
    #C_mat_X1 = C_mat[:p_1,:]
      
    if P_set == 0:              
        return X_1_hat, X_2_hat, C_1_hat, C_2_hat, D_1_hat, D_2_hat, r_1, r_2, r_12, D_theta[:r_12], np.arccos(D_theta[:r_12])/math.pi*180, C_mat, C_mat_neg, lambda_B, np.arccos(lambda_B)/math.pi*180
    else:
        return X_1_hat, X_2_hat, C_1_hat, C_2_hat, D_1_hat, D_2_hat, r_1, r_2, r_12, D_theta[:r_12], np.arccos(D_theta[:r_12])/math.pi*180, C_mat, C_mat_neg, lambda_B, np.arccos(lambda_B)/math.pi*180, P_mat                  
...
'''        
    ...
    if P_set == 0:
        C_mat = 0.5 * C_B @ (V_B1.T @ B_1 + V_B2.T @ B_2) @ C_base   
        C_mat_neg = 0.5 * C_B @ (V_B1.T @ B_1 - V_B2.T @ B_2) @ C_base 
        #C_mat_X2 = C_mat[:p_2,:]
    else:
        C_mat = 0.5 * C_B @ (V_B1.T @ B_1 + V_B2.T @ P_mat @ B_2) @ C_base
        C_mat_neg = 0.5 * C_B @ (V_B1.T @ B_1 - V_B2.T @ P_mat @ B_2) @ C_base
        #C_mat_X2 = P_mat.T[:p_2,:] @ C_mat
